...
for d in [2, 5, 10, 20]:
    ...
    for c in range (1,10):
         r = 0.01* (2.00**c)
         SVCModel =SVC(C=r, kernel ='poly', degree = d, gamma='auto')
         SVC1 = SVCModel.fit(st1,trainDigits)
    ...
    print("For degree", d, "the lowest c value is", LowestC, "for CFVE =",min(Error1))
    show()
'''
#Plot the decision region for the model with the best degree and the minimum error value
SVCModel =SVC(C=3988.590619269027, kernel ='poly', degree = 5, gamma='auto')
SVC1 = SVCModel.fit(st1,trainDigits)
...
#Graduate Question - Incr C will 
SVCModel =SVC(C=2, kernel ='poly', degree = 5, gamma='auto')
SVC1 = SVCModel.fit(st1,trainDigits)
